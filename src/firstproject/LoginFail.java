...
public class LoginFail {
    ...
    static String subFolderName = LoginFail.class.getSimpleName();
    static String fullFolderPath = baseFolder + File.separator + subFolderName;

    // Fungsi untuk mengambil screenshot ke dalam folder dinamis
    public static void takeScreenshot(WebDriver driver, String fileName) {
        try {
            File folder = new File(fullFolderPath);
            if (!folder.exists()) {
                folder.mkdirs(); // Buat folder utama + subfolder jika belum ada
            }
    ...
    public static void main(String[] args) {
        ...
        // Username
        try {
            driver.findElement(By.id("user-name")).sendKeys("testing aja");
